    ...
    <div className={`chat-client ${className || ''}`}>
      ...
      <div className="flex items-center justify-between p-4 border-b border-gray-200 dark:border-gray-700">
        <div className="flex items-center space-x-3">
          <h2 className="text-lg font-semibold text-gray-900 dark:text-white">المحادثة</h2>
          {selectedModel && (
        ...
        <div className="flex items-center space-x-2">
          <button onClick={clearChat} className="px-3 py-1 text-xs text-red-600 hover:bg-red-50 dark:hover:bg-red-900/20 rounded">
            مسح المحادثة
          </button>
        </div>
      </div>
      ...
      {availableModels.length > 0 && (
        <div className="p-4 border-b border-gray-200 dark:border-gray-700">
          <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">اختيار النموذج:</label>
          <div className="flex flex-wrap gap-2">
            {availableModels.map((model) => (
              ...
              >
                {model.name}
                {model.isLocal && <span className="ml-1 text-[10px]">(محلي)</span>}
              </button>
            ))}
          </div>
        </div>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4 min-h-[400px]">
        {messages.length === 0 ? (
          <div className="text-center py-12 text-gray-500 dark:text-gray-400">
            ...
            <p className="text-sm">{selectedModel ? `ابدأ المحادثة مع ${selectedModel.name}` : 'اختر نموذج ذكاء اصطناعي للبدء في المحادثة'}</p>
          </div>
        ) : (
            ...
            <div key={message.id} className={`flex ${message.role === 'user' ? 'justify-end' : 'justify-start'}`}>
              ...
              >
                <div className="flex items-start space-x-2">
                  <div className="flex-shrink-0">{message.role === 'user' ? <div className="i-ph:user text-lg" /> : <div className="i-ph:robot text-lg" />}</div>
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center justify-between mt-2 text-xs opacity-70">
                      ...
                      {message.modelId && (
                        <span className="text-xs">{availableModels.find((m) => m.id === message.modelId)?.name || 'نموذج غير معروف'}</span>
                      )}
                    </div>
                  </div>
                </div>
      ...
      {/* Error Display */}
      {error && <Alert type="error" message={error} onClose={() => chatActions.setError(null)} className="m-4" />}
        ...
        <div className="flex items-end space-x-3">
          <div className="flex-1">
            <textarea
              ...
              placeholder={selectedModel ? `اكتب رسالتك هنا... (${selectedModel.name})` : 'اختر نموذج ذكاء اصطناعي أولاً...'}
            ...
            <div className="flex justify-between items-center mt-1">
              ...
              <span className="text-xs text-gray-500">اضغط Enter للإرسال، Shift+Enter للسطر الجديد</span>
            </div>
          </div>

          <button onClick={handleSendMessage} disabled={!input.trim() || !selectedModel || isLoading} className="px-4 py-2 bg-blue-500 text-white rounded-lg hover:bg-blue-600 disabled:opacity-50 disabled:cursor-not-allowed transition-colors">
            {isLoading ? (
              <div className="flex items-center space-x-2">
