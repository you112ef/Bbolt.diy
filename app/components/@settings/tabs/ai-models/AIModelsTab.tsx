...
const validateModelFile = async (file: File): Promise<ModelValidation> => {
  ...
  if (fileName.endsWith('.gguf')) {
    ...
    if (fileName.includes('7b')) {
      estimatedParameters = '7B';
    } else if (fileName.includes('13b')) {
      estimatedParameters = '13B';
    } else if (fileName.includes('70b')) {
      estimatedParameters = '70B';
    } else if (fileName.includes('3b')) {
      estimatedParameters = '3B';
    }
    ...
    <div className="space-y-3 sm:space-y-4 lg:space-y-6">
      ...
      <div>
        ...
        <p className="text-xs sm:text-sm text-bolt-elements-textSecondary">
          Upload and manage local AI models for offline inference. Models are stored securely on your device and work
        ...
        <div className="space-y-2 sm:space-y-3 lg:space-y-4">
          ...
          >
            <div className="space-y-1 sm:space-y-2">
              <div
                className={`text-2xl sm:text-3xl lg:text-4xl transition-transform ${
              ...
              {isMobile() && (
                <div className="text-[10px] sm:text-xs text-bolt-elements-accent">📱 Mobile-optimized storage</div>
              )}
            </div>
          </div>
        ...
        {localModels.length === 0 ? (
          <div className="text-center py-4 sm:py-6 lg:py-8 text-bolt-elements-textSecondary">
            ...
            <p className="text-[10px] sm:text-xs mt-1">
              Upload your first model to get started with offline AI inference
            </p>
          </div>
        ) : (
                ...
                <div className="flex items-start justify-between gap-3">
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="text-xs sm:text-sm text-bolt-elements-textSecondary space-y-0.5">
                      <p>
                        {model.type} • {model.parameters} • {(model.size / (1024 * 1024)).toFixed(0)}MB
                      </p>
                      <p>
                        {model.architecture} • Uploaded {new Date(model.uploadDate).toLocaleDateString()}
                      </p>
                      {model.capabilities && (
      ...
      <Card className="p-3 sm:p-4 lg:p-6 bg-blue-50 dark:bg-blue-950/20 border-blue-200 dark:border-blue-800">
        ...
        <div className="space-y-1 sm:space-y-2 text-[10px] sm:text-xs lg:text-sm text-blue-800 dark:text-blue-200">
          ...
          {isMobile() && <p>• On mobile: Models are stored in Documents/BoltDIY/AI_Models/</p>}
        </div>

        {localModels.length > 0 && (
          <div className="mt-3 sm:mt-4 p-2 sm:p-3 bg-blue-100 dark:bg-blue-900/30 rounded-lg">
            <h4 className="text-xs sm:text-sm font-medium text-blue-900 dark:text-blue-100 mb-1">📊 Storage Usage</h4>
            <div className="text-[10px] sm:text-xs text-blue-800 dark:text-blue-200">
              <p>Total Models: {localModels.length}</p>
              <p>
                Total Storage:{' '}
                {(localModels.reduce((sum, model) => sum + model.size, 0) / (1024 * 1024 * 1024)).toFixed(2)}GB
              </p>
